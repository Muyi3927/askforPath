...
export const PostDetail: React.FC<PostDetailProps> = ({ posts, updatePost, categories }) => {
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      // If logic handles loading state in App, this redirection might happen too fast on refresh. 
      // But if App handles it, props.posts will be empty initially.
      // We assume App shows loading spinner until data is ready.
    }
  }, [id, posts]);

  if (!post) {
      // Simple check: if posts are empty, we might be loading or it's a 404.
      // Since App handles "loading" state, if we reach here with empty posts, it's likely 404.
      if (posts.length > 0) {
          return <div className="text-center py-20">文章未找到</div>;
      }
  ...
  const handleDelete = async () => {
    if (confirm("确定要删除这篇文章吗？此操作不可恢复。")) {
      try {
        ...
        alert("文章已删除");
        // 简单刷新页面或跳转首页，实际项目中应通知父组件移除 post
            ...
            {isAdmin && (
                <div className="flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className="flex items-center text-red-600 hover:text-red-700 font-bold text-sm bg-red-50 dark:bg-red-900/30 px-3 py-1 rounded-full"
                    >
