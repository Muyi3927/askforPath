...
import { BlogPost, Category, UserRole } from '../types';
...
export const Editor: React.FC<EditorProps> = ({ onSave, categories, onAddCategory, onDeleteCategory, posts }) => {
  ...
  useEffect(() => {
    if (!id) {
      ...
      if (post) {
        ...
        setCategoryId(post.categoryId || (categories.length > 0 ? categories[0].id : ''));
  ...
  const handleSave = async () => {
    ...
    const newPost: BlogPost = {
      ...
      author: user || { id: 'admin-fallback', username: 'Admin', role: UserRole.ADMIN },
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      await api.createCategory(newCategoryName.trim(), parent);
      
      alert("分类添加成功！");
      setNewCategoryName('');
      window.location.reload(); // 刷新页面以更新分类树（简单方式）
    } catch (err) {
      alert("添加失败");
    }
  };
